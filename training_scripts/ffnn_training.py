...
from M2F.pyg_data_interfaces import DatasetInput, ProteinDataset
import M2F.wb as wb
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train an FFNN classifier on M2F protein sequence features."
    )
    ...
    parser.add_argument("--num-feature-batches", type=int, default=1)
    ...
    if args.num_feature_batches < 1:
        parser.error("--num-feature-batches must be >= 1.")
    return args
...
def main() -> None:
    ...
    try:
        with ProteinDataset(
            ...
            model_config = {
                "num_nodes": int(ds.meta["num_nodes"]),
                "in_dim": in_dim,
                "out_dim": out_dim,
            ...
            train_loader = ds.train_loader(batch_size=args.batch_size, shuffle=True)
            val_loader = ds.val_loader(batch_size=args.batch_size)
            test_loader = ds.test_loader(batch_size=args.batch_size)
