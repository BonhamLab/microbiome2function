...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a GraphConv node classifier on an M2F protein co-occurrence graph."
    )
    ...
    parser.add_argument(
        "--edge-features-used-as",
        choices=("scaling", "catting"),
        default="scaling",
    )
...
def main() -> None:
    ...
    if edge_dim == 0 and args.edge_features_used_as == "scaling":
        raise ValueError("GraphConv scaling mode requires at least one edge attribute.")
    ...
    try:
        ...
        model = GraphConvNodeClassifier(
            ...
            out_dim=out_dim,
            edge_features_used_as=args.edge_features_used_as,
